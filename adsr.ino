/**
 * @file DCO-adsr.ino
 * @brief ADSR Envelope Management, Voice Modulation Routing, and Trigger Engine.
 * 
 * Handles per-voice amplitude (VCA) and pitch/PWM (DCO/ADSR3) envelopes, as well
 * as shared paraphonic filter (VCF1/VCF2) envelopes. Implements high-speed 
 * interleaved execution (~10 kHz on Core 1) and deferred parameter updating (~200 Hz).
 */
 ...
 /** @brief Legacy 12-bit DAC output levels for DCO pitch/PWM envelope (per voice). */
 uint16_t ADSR3Level[NUM_VOICES_TOTAL];
 
 /** @brief Legacy 12-bit DAC output levels for VCA amplitude envelope (per voice). */
 ...
 /** @brief Primary Q15 modulation output tap (0 .. 32767) for DCO envelope (per voice). */
 int16_t ADSR3Level_q15[NUM_VOICES_TOTAL];
 
 /** @brief Primary Q15 modulation output tap (0 .. 32767) for VCA envelope (per voice). */
 ...
 /** @brief Volatile Q15 output level read by audio synthesis tasks for DCO envelope. */
 volatile int16_t ADSR3Level_q15_volatile[NUM_VOICES_TOTAL];
 
 /** @brief Volatile Q15 output level read by audio synthesis tasks for VCA envelope. */
 volatile int16_t ADSR_VCA_Level_q15_volatile[NUM_VOICES_TOTAL];
 
 /** @brief Volatile Q15 output level read by audio synthesis tasks for VCF1 envelope. */
 volatile int16_t ADSR_VCF_Level_q15_volatile;
 
 /** @brief Volatile Q15 output level read by audio synthesis tasks for VCF2 envelope. */
 ...
 /** @brief Note-on edge triggers set by MIDI parser (1 = Trigger noteOn, cleared by engine). */
 volatile byte noteStart[NUM_VOICES_TOTAL];
 
 /** @brief Note-off edge triggers set by MIDI parser (1 = Trigger noteOff, cleared by engine). */
 volatile byte noteEnd[NUM_VOICES_TOTAL];
 
 /** @brief Tracks count of actively held keys for paraphonic filter envelope management. */
 volatile uint8_t vcf_active_gates = 0;
 
 /** 
  * @brief Current triggering mode for shared filter envelopes.
  * @details Default is VCF_TRIGGER_PARAPHONIC_MULTI (punchy per-note attack, release on last key).
  */
 ...
 /** @brief Target oscillator routing for DCO envelope: 0=OSC1, 1=OSC2, 2=OSC1+2, 3=OSC3, 4=All. */
 int8_t ADSR3ToOscSelect = 2;
 
 /** @brief EnvDCO pitch tap polarity: 0 = unipolar (0..1), 1 = centered (-1..+1 around mid-sustain). */
 ...
 /** @brief Retrigger mode for DCO envelope (true = start from 0, false = analog ramp from current level). */
 ...
 /** @brief Raw pulse-width modulation depth parameter for DCO envelope (-512 .. +511). */
 int16_t ADSR3toPWM;
 
 /** @brief Full-scale pulse-width count delta for DCO envelope modulation. */
 int32_t ADSR3toPWM_scale = 0;
 
 /** @brief Bitmask of modified ADSR parameters waiting to be pushed to Core 1 voices. */
 ...
 adsr adsr3_voice_0(ADSR_1_CC, ADSR_CURVE_LINEAR, ADSR_CURVE_LINEAR, ADSR_CURVE_LINEAR);
 /** @brief DCO pitch/PWM envelope generator for Voice 1. */
 adsr adsr3_voice_1(ADSR_1_CC, ADSR_CURVE_LINEAR, ADSR_CURVE_LINEAR, ADSR_CURVE_LINEAR);
 /** @brief DCO pitch/PWM envelope generator for Voice 2. */
 adsr adsr3_voice_2(ADSR_1_CC, ADSR_CURVE_LINEAR, ADSR_CURVE_LINEAR, ADSR_CURVE_LINEAR);
 /** @brief DCO pitch/PWM envelope generator for Voice 3. */
 adsr adsr3_voice_3(ADSR_1_CC, ADSR_CURVE_LINEAR, ADSR_CURVE_LINEAR, ADSR_CURVE_LINEAR);
 ...
 adsr adsr_vcf_voice(ADSR_CV_CC, ADSR_CURVE_S_CURVE_SOFT, ADSR_CURVE_ROUNDED, ADSR_CURVE_EXP_SMOOTH);
 
 /** @brief Shared secondary filter (VCF2) envelope generator. */
 adsr adsr_vcf2_voice(ADSR_CV_CC, ADSR_CURVE_S_CURVE_SOFT, ADSR_CURVE_ROUNDED, ADSR_CURVE_EXP_SMOOTH);
 ...
 /**
  * @brief Converts panel MIDI/CC sustain level (0 .. panel_full) into native envelope scale.
  * 
  * @param panel      Raw incoming panel/CC sustain value (e.g. 0 .. 4095).
  * @param panel_full Full-scale panel reference (typically ADSR_CV_SCALE = 4096).
 ...
 /**
  * @brief Set the triggering and release behavior mode for shared filter envelopes.
  * 
  * ### Mode Options:
 ...
 /**
  * @brief Boot Initialization. Generates global Bézier tables and assigns initial envelope settings.
  */
 void init_ADSR() {
 #if ADSR_BEZIER_NATIVE_Q15
 ...
 void __not_in_flash_func(ADSR_update)() {
   ...
   for (int i = phase; i < NUM_VOICES; i += 2) {
     ...
     if (noteEnd[i] == 1) {
       ...
       if (vcf_trigger_mode == VCF_TRIGGER_DIRECT_PER_VOICE) {
         // Direct mode: any voice release sends noteOff immediately
         adsr_vcf_voice.noteOff();
         adsr_vcf2_voice.noteOff();
       } else {
         // Paraphonic modes: only enter release when all held keys are released
         if (vcf_active_gates > 0) vcf_active_gates--;
         if (vcf_active_gates == 0) {
           adsr_vcf_voice.noteOff();
           adsr_vcf2_voice.noteOff();
         }
       }
     ...
     // -------------------------------------------------------------------------
       ...
       if (vcf_trigger_mode == VCF_TRIGGER_PARAPHONIC_LEGATO) {
         // Legato: Trigger attack only if no other keys were already held
         if (vcf_active_gates == 0) {
       ...
       else if (vcf_trigger_mode == VCF_TRIGGER_PARAPHONIC_MULTI) {
         // Multi-trigger: Retrigger attack on every note, track gates for release
         adsr_vcf_voice.noteOn();
         adsr_vcf2_voice.noteOn();
         vcf_active_gates++;
       } 
       else { // VCF_TRIGGER_DIRECT_PER_VOICE
         // Direct: Fire noteOn immediately
         adsr_vcf_voice.noteOn();
         adsr_vcf2_voice.noteOn();
       }
     }
 
     // Sample active voice outputs
 #if ADSR_BEZIER_NATIVE_Q15
   ...
   // Sample shared filter envelopes outside the interleaved loop (evaluated every tick)
 #if ADSR_BEZIER_NATIVE_Q15
 ...
 inline void ADSR_set_parameters() {
   ...
   if (++tick < 50) return; // Prescale to ~200 Hz
   tick = 0;
   ...
   if (ch & ADSR_DIRTY_DCO_ALL) {
     ...
     for (int i = 0; i < NUM_VOICES; i++) {
       if (ch & ADSR_DIRTY_DCO_A) ADSRVoices[i].adsr3_voice.setAttack(ADSR3_attack);
       if (ch & ADSR_DIRTY_DCO_D) ADSRVoices[i].adsr3_voice.setDecay(ADSR3_decay);
       if (ch & ADSR_DIRTY_DCO_S) ADSRVoices[i].adsr3_voice.setSustain(s);
       if (ch & ADSR_DIRTY_DCO_R) ADSRVoices[i].adsr3_voice.setRelease(ADSR3_release);
     }
   }
 ...
 /**
  * @brief Set the Attack curve shape preset for all VCA envelopes across active voices.
 ...
 /**
  * @brief Set the Decay curve shape preset for all VCA envelopes across active voices.
 ...
 /**
  * @brief Set the Release curve shape preset for all VCA envelopes across active voices.
 ...
 /**
  * @brief Set the Attack curve shape preset for the primary filter (VCF1) envelope.
  * @param adsrCurveAttack Curve preset index (0 to 7) or ADSRCurveType.
  */
 ...
 /**
  * @brief Set the Decay curve shape preset for the primary filter (VCF1) envelope.
  * @param adsrCurveDecay Curve preset index (0 to 7) or ADSRCurveType.
  */
 ...
 /**
  * @brief Set the Release curve shape preset for the primary filter (VCF1) envelope.
  * @param adsrCurveRelease Curve preset index (0 to 7) or ADSRCurveType.
  */
 ...
 /**
  * @brief Set the Attack curve shape preset for the secondary filter (VCF2) envelope.
  * @param adsrCurveAttack Curve preset index (0 to 7) or ADSRCurveType.
  */
 ...
 /**
  * @brief Set the Decay curve shape preset for the secondary filter (VCF2) envelope.
  * @param adsrCurveDecay Curve preset index (0 to 7) or ADSRCurveType.
  */
 ...
 /**
  * @brief Set the Release curve shape preset for the secondary filter (VCF2) envelope.
  * @param adsrCurveRelease Curve preset index (0 to 7) or ADSRCurveType.
  */
 ...
 /**
  * @brief Set the Attack curve shape preset for all DCO envelopes across active voices.
  * @param adsrCurveAttack Curve preset index (0 to 7) or ADSRCurveType.
  */
 ...
 /**
  * @brief Set the Decay curve shape preset for all DCO envelopes across active voices.
  * @param adsrCurveDecay Curve preset index (0 to 7) or ADSRCurveType.
  */
 ...
 /**
  * @brief Set the Release curve shape preset for all DCO envelopes across active voices.
  * @param adsrCurveRelease Curve preset index (0 to 7) or ADSRCurveType.
  */
