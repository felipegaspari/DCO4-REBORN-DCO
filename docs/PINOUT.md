...
| ~~Screen (direct)~~ | *(removed)* | *(GP8/9 now spare)* | 2 500 000 | Was SerialPIO GP8/9; deleted — gap reaches Screen through Input |
...
**Current:** DIN on HW UART0 @ GP0/1 (`Serial1` in Arduino-Pico); Input on HW UART1 @ GP20/21 (`Serial2`). Both hardware UARTs are spoken for, and the DCO has no Screen port at all — gap display goes out on the Input link and Input forwards it on its own `Serial2`. GP8/9 are free.
...
| 5 | Resonance 0 PWM |
| 7 | Resonance 1 PWM |
| 8,9 | Spare (was the SerialPIO Screen UART, now removed) |
| 10 | Cal sense |
| 11 | VCA PWM |
| 12–14 | 74HC595 |
| 15 | Cutoff 0 PWM |
